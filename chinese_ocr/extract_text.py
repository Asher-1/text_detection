...
class ImageTableOCR(object):
    ...
    def HorizontalLineDetect(self):
        ...
        # thresh1 = cv2.adaptiveThreshold(self.gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 7, -1)
        # cv2.imwrite('./test_result/gray0.png', thresh1)
        # 进行两次中值滤波
    ...
    def horizontalline_detect(self):
        thresh1 = cv2.adaptiveThreshold(self.gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 7, -2)
        horizontal_size = thresh1.shape[1] // self.scale
        kernel_row = np.ones((1, horizontal_size))
        img_open_row = cv2.morphologyEx(thresh1, cv2.MORPH_OPEN, kernel_row)
        return img_open_row

    def verticalline_detect(self):
        # ret, thresh1 = cv2.threshold(self.gray, 240, 255, cv2.THRESH_BINARY_INV)
        thresh1 = cv2.adaptiveThreshold(self.gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 7, -2)
        vertical_size = thresh1.shape[0] // self.scale
        kernel_col = np.ones((vertical_size, 1))
        img_open_col = cv2.morphologyEx(thresh1, cv2.MORPH_OPEN, kernel_col)
        return img_open_col

    def get_mask(self):
        return self.horizontalline_detect() + self.verticalline_detect()

    def get_vertex(self):
        h_mask = self.horizontalline_detect()
        v_mask = self.verticalline_detect()
        vertex = np.zeros(shape=v_mask.shape, dtype=v_mask.dtype)
        cv2.bitwise_and(h_mask, v_mask, vertex)
        return vertex
...
ImagePath = './test_images/demo2.png'
imageOCR = ImageTableOCR(ImagePath)

mask = imageOCR.get_mask()
vertex = imageOCR.get_vertex()
cv2.imwrite("./test_result/mask.png", mask)
cv2.imwrite("./test_result/vertex.png", vertex)
imageOCR.VertexDetect()
imageOCR.SaveImage(path='./test_result/demo3_res.png')
imageOCR.OCR()
