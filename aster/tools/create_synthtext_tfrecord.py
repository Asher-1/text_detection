...
ROOT_PATH = "/media/yons/data/dataset/images/text_data/SynthText/SynthText"
OUT_PATH = "/media/yons/data/dataset/images/text_data/SynthText/synthtext2.tfrecord"
...
flags.DEFINE_integer('num_dump_images', 10, 'Number of images to dump for debugging')
FLAGS = flags.FLAGS
...
class SynthTextCreator(object):
    ...
    def _create_samples_of_an_image(self, image_rel_path, words, word_polygons, char_polygons):
        """
        Args:
...
def create_samples_of_an_image(writer, image_rel_path, words, word_polygons, char_polygons):
    """
    Args:
      image_rel_path: Python string. Image relative path.
      words: string array of shape [num_words]. Groundtruth words.
...
def main(_):
    ...
    #     word_polygons = groundtruth['wordBB'][0, index]
    #     char_polygons = np.transpose(groundtruth['charBB'][0, index], axes=[2, 1, 0])
    #     create_samples_of_an_image(writer, image_rel_path, text, word_polygons, char_polygons)
    ...
    for index in tqdm(indices):
        ...
        if not os.path.exists(image_path):
            print("cannot find image : ".format(image_path))
            continue
