...
train_nips = "/media/yons/data/dataset/images/text_data/FOTS_TF/lmdb"
train_cvpr = "/media/yons/data/dataset/images/text_data/chinese_ocr_data/syn_train"
# train_cvpr = "/media/yons/data/dataset/images/text_data/MORAN/IIIT5K_3000"
valroot = "/media/yons/data/dataset/images/text_data/chinese_ocr_data/syn_test"
# valroot2 = "/media/yons/data/dataset/images/text_data/MORAN/IIIT5K_3000"
valroot2 = "/media/yons/data/dataset/images/text_data/FOTS_TF/lmdb"

continue_training = True
# MORAN_MODEL = "/media/yons/data/dataset/images/text_data/MORAN/demo.pth"
MORAN_MODEL = "/home/yons/develop/AI/text_detection/MORAN_v2/output/5000_0.0610.pth"

os.environ['CUDA_VISIBLE_DEVICES'] = "0"

parser = argparse.ArgumentParser()
parser.add_argument('--train_nips', default=train_nips, help='path to dataset')
parser.add_argument('--train_cvpr', default=train_cvpr, help='path to dataset')
parser.add_argument('--valroot', default=valroot, help='path to dataset')
...
parser.add_argument('--lr', type=float, default=0.01, help='learning rate for Critic, default=0.00005, 0.01')
parser.add_argument('--cuda', action='store_true', default=True, help='enables cuda')
parser.add_argument('--ngpu', type=int, default=1, help='number of GPUs to use')
parser.add_argument('--MORAN', default=MORAN_MODEL, help="path to model (to continue training)")
...
parser.add_argument('--saveInterval', type=int, default=16400, help='Interval to be displayed')
parser.add_argument('--adam', action='store_true', default=False, help='Whether to use adam (default is rmsprop)')
...
parser.add_argument('--sgd', action='store_true', default=False, help='Whether to use sgd (default is rmsprop)')
parser.add_argument('--BidirDecoder', action='store_true', default=True, help='Whether to use BidirDecoder')
...
# train 2
# train_nips2_dataset = dataset.lmdbDataset(root=train_nips2,
#                                           transform=dataset.resizeNormalize((opt.imgW, opt.imgH)),
#                                           reverse=opt.BidirDecoder,
#                                           alphabet=opt.alphabet)
# assert train_nips2_dataset
# val
...
test_dataset = dataset.lmdbDataset(root=opt.valroot,
                                   ...
                                   alphabet=opt.alphabet)
assert test_dataset
test_dataset2 = dataset.lmdbDataset(root=valroot2,
                                    transform=dataset.resizeNormalize((opt.imgW, opt.imgH)),
                                    reverse=opt.BidirDecoder,
                                    alphabet=opt.alphabet)
assert test_dataset2
test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset2])

alphabet = [alpha for alpha in opt.alphabet]
nclass = len(alphabet)
# nclass = len(opt.alphabet.split(opt.sep))
nc = 1

converter = utils.strLabelConverterForAttention(alphabet, opt.sep)
...
if opt.MORAN != '':
    ...
    for k, v in state_dict.items():
        ...
        if not continue_training:
            if name in ["ASRN.attentionL2R.char_embeddings", "ASRN.attentionL2R.generator.weight",
                        "ASRN.attentionL2R.generator.bias", "ASRN.attentionR2L.char_embeddings",
                        "ASRN.attentionR2L.generator.weight", "ASRN.attentionR2L.generator.bias"]:
                continue
        MORAN_state_dict_rename[name] = v
    MORAN.load_state_dict(MORAN_state_dict_rename, strict=False)
...
def val(dataset, criterion, max_iter=1000):
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, shuffle=True, batch_size=opt.batchSize, num_workers=int(opt.workers))  # opt.batchSize
    ...
    for i in range(max_iter):
        ...
        if opt.BidirDecoder:
            ...
            for j in range(cpu_images.size(0)):
                ...
                list_1 = preds0_prob[text_begin:text_begin + len(sim_preds0[j].split('$')[0] + '$')]
                list_2 = preds1_prob[text_begin:text_begin + len(sim_preds1[j].split('$')[0] + '$')]
                # if torch.mean(list_1).data[0] > torch.mean(list_2).data[0]:
                if torch.mean(list_1).item() > torch.mean(list_2).item():
...
for epoch in range(opt.niter):
    ...
    while i < len(train_loader):
        ...
        if i % opt.saveInterval == 0:
            torch.save(MORAN.state_dict(), '{0}/{1}_{2}.pth'.format(opt.experiment, epoch, i))
