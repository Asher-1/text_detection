    ...
    def roi_rotate_tensor_while(self, feature_map, transform_matrixs, box_masks, box_widths, is_debug=False):
        ...
        with tf.variable_scope("RoIrotate"):
            ...
            def body(pad_rois, i):
                ...
                roi = tf.image.crop_and_resize(after_transform, [[0, 0, 8 / map_shape[0], width_box / map_shape[1]]],
                                               [0], [8, tf.cast(width_box, tf.int32)])
...
def check_RoIRotate(RR):
    ...
    with tf.Session() as sess:
        inp_dict = {input_feature_map: data[0], input_feature_height: data[1], input_feature_width: data[2],
                    input_transform_matrix: data[3], input_box_widths: data[5]}
        for i in range(2):
