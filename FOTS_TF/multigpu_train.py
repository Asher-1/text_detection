...
ROOT_PATH = "/media/yons/data/dataset/models/text_detection_models/FOTS_TF/"
CHECKPOINT_PATH = ROOT_PATH + "checkpoint"
pretrained_model_path = ROOT_PATH + "pretrained_model/model.ckpt-44441"

tf.app.flags.DEFINE_integer('input_size', 512, '')
# tf.app.flags.DEFINE_integer('batch_size_per_gpu', 12, '')
tf.app.flags.DEFINE_integer('batch_size_per_gpu', 12, '')
tf.app.flags.DEFINE_integer('num_readers', 10, '')
tf.app.flags.DEFINE_float('learning_rate', 0.01, '')  # 0.0001
tf.app.flags.DEFINE_integer('max_steps', 900001, '')
...
tf.app.flags.DEFINE_boolean('new_data', True, 'new data')
...
def main(argv=None):
    ...
    if FLAGS.pretrained_model_path is not None:
        if os.path.isdir(FLAGS.pretrained_model_path):
            ...
            if FLAGS.new_data:
                trainable_variables = [var for var in trainable_variables if var.name not in ["W:0", "b:0"]]
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        if FLAGS.restore:
            ...
            print('continue training from previous checkpoint : {}'.format(ckpt))
