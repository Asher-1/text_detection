...
CHECKPOINT_PATH = "/media/yons/data/dataset/models/text_detection_models/FOTS_TF/checkpoint"
IMAGE_PATH = "./demo_images/license/"
...
tf.app.flags.DEFINE_string('test_data_path', IMAGE_PATH, '')
...
tf.app.flags.DEFINE_bool('use_vacab', False, 'strong, normal or weak')
...
def cv2ImgAddText(img, text, left, top, textColor=(0, 255, 0), textSize=20):
    ...
    fontStyle = ImageFont.truetype(
        "ukai.ttc", textSize, encoding="utf-8")
    # 绘制文本
...
def main(argv=None):
    ...
    with tf.get_default_graph().as_default():
        ...
        pad_rois = roi_rotate_part.roi_rotate_tensor_pad(shared_feature, input_transform_matrix, input_box_mask,
                                                         input_box_widths)
                    ...
                    with open(res_file, 'w') as f:
                        for i, box in enumerate(boxes):
                            ...
                            cv2.fillPoly(im[:, :, ::-1], [text_area.astype(np.int32).reshape((-1, 1, 2))],
                                         color=(0, 255, 0))
                            im_txt = cv2ImgAddText(im[:, :, ::-1], recognition_result, box[0, 0], box[0, 1],
                                                   textColor=(0, 0, 255), textSize=20)
                            # im_txt = cv2.putText(im[:, :, ::-1], recognition_result, (box[0, 0], box[0, 1]), font, 0.5,
                            #                      (0, 0, 255), 1)
                else:
                    timer['net'] = time.time() - start
