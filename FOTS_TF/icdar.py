...
# tf.app.flags.DEFINE_string('training_data_path', '/data2/data/15ICDAR/ch4_training_images/',
# 'training dataset to use')
tf.app.flags.DEFINE_string('training_data_path', TRAIN_PATH,
                           'training dataset to use')
...
def label_to_array(label):
    try:
        ...
        return [config.CHAR_VECTOR.index(x) for x in label if x in config.CHAR_VECTOR]
...
def check_and_validate_polys(polys, tags, xxx_todo_changeme):
    ...
    for poly, tag in zip(polys, tags):
        ...
        if abs(p_area) < 1:
            # print poly
            print('invalid poly', poly, " p_area: {}".format(p_area))
            continue
        if p_area > 0:
...
def sort_rectangle(poly):
    ...
    if np.count_nonzero(poly[:, 1] == poly[p_lowest, 1]) == 2:
        ...
        # assert angle > 0
        if angle <= 0:
            print("invalid angle: {}\t poly_lowest_point: {}\t poly_lowest_right: {}".format(angle, poly[p_lowest],
                                                                                             poly[p_lowest_right]))
    ...
    while True:
        ...
        for i in index:
            try:
                ...
                txt_fn = os.path.join(TRAIN_PATH, child_name)
                ...
                if np.random.rand() < background_ratio:  # Since the background_ratio is 0 so it won't dive in this branch
                    ...
                    im, text_polys, text_tags, selected_poly = crop_area(im, text_polys, text_tags,
