        ...
        try:
            ...
            with open(label_file, "w", encoding="utf-8") as f:
                for block in anno['annotations']:
                    words = []
                    char_polygons_list = []
                    left_top = block[0]['polygon'][0]
                    left_bottom = block[0]['polygon'][3]
                    right_top = block[-1]['polygon'][1]
                    right_bottom = block[-1]['polygon'][2]
                    polygon = [left_top, right_top, right_bottom, left_bottom]
                    polygon = verify_coordinates(polygon, image_h, image_w)

                    poly_array = check_and_validate_polys(polygon)
                    if poly_array is None:
                        invalid_img = cv2.imread(image_path)
                        location = np.array(polygon).flatten()
                        cv2.polylines(invalid_img, [np.int0(location).reshape((-1, 1, 2))], True, (0, 255, 0), 1)
                        cv2.imwrite(os.path.join(VIS_PATH, anno['file_name']), invalid_img)
                        continue
                    ...
                    difficulty = "1" if "#" in label else "0"
            ...
            if empty:
                ...
                if VIS:
                    cv2.imwrite(os.path.join(VIS_PATH, anno['file_name']), img)
                count += 1
...
if __name__ == '__main__':
    ...
    IMAGE_DIR = os.path.join(ROOT_PATH, "images/trainval")
    IMAGE_TEST_DIR = os.path.join(ROOT_PATH, "images/test")

    # info_annatations = os.path.join(ROOT_PATH, "ctw_annotations/info.jsonl")
    train_annatations = os.path.join(ROOT_PATH, "ctw_annotations/train.jsonl")
    val_annatations = os.path.join(ROOT_PATH, "ctw_annotations/val.jsonl")
    test_annatations = os.path.join(ROOT_PATH, "ctw_annotations/test_cls.jsonl")
    ...
    with open(val_annatations) as f:
        groundtruth += f.read().splitlines()
    # with open(test_annatations) as f:
    #     groundtruth = f.read().splitlines()

    num_images = len(groundtruth) - start_index
    if num_images > 0:
        num_images = min(num_images, num_images)
