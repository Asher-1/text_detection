...
from application import trainTicket, idcard, BusinessLicense
from apphelper.image import union_rbox, adjust_box_to_origin, base64_to_PIL


def init_env(filelock, LSTMFLAG):
    ...
    if yoloTextFlag == 'opencv':
        ...
        raise ValueError(r"err,text engine in keras\opencv\darknet")
    if ocr_redis:
        # 多任务并发识别
        ...
        if ocrFlag == 'keras':
            ...
            raise ValueError(r"err,ocr engine in keras\opencv\darknet")
    ...
    return scale, maxScale, crnn, model
...
scale, maxScale, crnn, model = init_env(filelock, LSTMFLAG)

billList = ['通用OCR', '火车票', '身份证', '营业执照']
...
class OCR:
    ...
    def POST(self):
        ...
        try:
            ...
            if len(res) < 1:
                return json.dumps({'res': res, 'timeTake': 0, 'state': '非{}'.format(billModel)},
                                  ensure_ascii=False, indent=4)

            return json.dumps({'res': res, 'timeTake': round(timeTake, 4), 'state': 'success'},
                              ensure_ascii=False, indent=4)
        except Exception as e:
            return json.dumps({'res': res, 'timeTake': 0, 'state': str(traceback.format_exc())},
                              ensure_ascii=False, indent=4)
