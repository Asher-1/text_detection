...
class CRNN(nn.Module):
    def __init__(self, imgH, nc, nclass, nh, leakyRelu=False, lstmFlag=True, GPU=False, alphabet=None):
        ...
        self.ctc_mode = False  # 应用语言模型， 结果会更糟
        ...
        self.ctc_decode = self.load_lm() if self.ctc_mode else None

    def load_lm(self):
        """
        加载语言模型
        参考：https://github.com/lukhy/masr/blob/master/beamdecode.py
        """
        from ctcdecode import CTCBeamDecoder
        alpha = 0.1
        beta = 0.1
        lm_path = '/media/yons/data/dataset/models/text_detection_models/' \
                  'chineseocr/models/zh_giga.no_cna_cmn.prune01244.klm'
        cutoff_top_n = 40
        cutoff_prob = 1.0
        beam_width = 32
        num_processes = 8
    ...
    def predict(self, image):
        ...
        if self.ctc_decode is not None and self.ctc_mode:
            preds = preds.permute(1, 0, 2)  # B * T * V
            preds = F.softmax(preds, 2)
            y_len = torch.tensor([preds.size(1)])
            out, score, offset, out_len = self.ctc_decode.decode(preds, y_len)
            out = out[0][0]
            out_len = out_len[0][0]
            return "".join([self.alphabet[x - 1] for x in out[0:out_len]])
        else:
            _, preds = preds.max(2)
