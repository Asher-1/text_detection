...
def translate_opts(parser, config):
    """ Translation / inference options """
    ##################################################
    src = config.IMAGE_INFO
    src_dir = config.TEMP_PATH
    model_file = config.MODEL_FILE
    log_file = config.MODEL_FILE
    ...
    group.add('--model', '-model', dest='models', metavar='MODEL',
              nargs='+', type=str, default=[model_file, ],
    ...
    group = parser.add_argument_group('Data')
    group.add('--data_type', '-data_type', default=data_type,
              help="Type of the source input. Options: [text|img].")

    group.add('--src', '-src', default=src,
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--src_dir', '-src_dir', default=src_dir,
              help='Source directory for image or audio files')
    group.add('--tgt', '-tgt',
    ...
    group.add('--shard_size', '-shard_size', type=int, default=10000,
              help="Divide src and tgt (if applicable) into "
                   ...
                   "shard_size>0 means segment dataset into multiple shards, "
                   "each shard has shard_size samples")
    group.add('--output', '-output', default=pre_path,
    ...
    group = parser.add_argument_group('Logging')
    group.add('--verbose', '-verbose', action="store_true", default=verbose,
              help='Print scores and predictions for each sentence')
    group.add('--log_file', '-log_file', type=str, default=log_file,
              help="Output logs to a file under this path.")
    group.add('--log_file_level', '-log_file_level', type=str,
              action=StoreLoggingLevelAction,
              choices=StoreLoggingLevelAction.CHOICES,
    ...
    group.add('--batch_size', '-batch_size', type=int, default=30,
              help='Batch size')
    group.add('--gpu', '-gpu', type=int, default=gpu_mode,
